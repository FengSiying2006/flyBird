...
Game::Game(QWidget* parent) : QGraphicsView(parent), score(0),isGameOver(false) {
    ...
    QGraphicsTextItem *memberInfo = new QGraphicsTextItem();
    memberInfo->setPlainText("小组成员：\n冯思颖  张心晨  吴袁瑾\n章谦知  韩耀栋  纪雨涵\n");
    memberInfo->setDefaultTextColor(Qt::white);
    memberInfo->setFont(QFont("Microsoft YaHei", 9, QFont::Bold));
    memberInfo->setZValue(10);
    memberInfo->setPos(270, 10);
    scene->addItem(memberInfo);
...
void Game::gameLoop() {
	...
	while (it != pipes.end()) {
		...
		if (bird->collidesWithItem(pipe)) {
			timer->stop();
            PlaySound(TEXT("assets\\sounds\\lose-game.wav"), NULL, SND_FILENAME | SND_ASYNC);
        ...
        if (pipe->x() + pipe->boundingRect().width() < bird->x() && !pipe->isPassed) {
            ...
            score++;
                if (score == 10) {
                // 1. 变身绿鸟
                bird->setSkin("greenbird");

                PlaySound(TEXT("assets\\sounds\\sfx_point.wav"), NULL, SND_FILENAME | SND_ASYNC);
                ...
                if (score == 20) {
                    // 1. 变身黄紫鸟
                    bird->setSkin("manbird");

                    // 2. 播放新音效
                    PlaySound(TEXT("assets\\sounds\\sfx_point_1.wav"), NULL, SND_FILENAME | SND_ASYNC);
